...
const LESSONS_FILE = process.env.MERIDIAN_LESSONS_FILE || "./lessons.json";
...
export async function recordPerformance(perf) {
  ...
  const entry = {
    ...
    paper_lifecycle: perf.paper_lifecycle || null,
...
export function evolveThresholds(perfData, config) {
  ...
  // If winners span higher volatility safely → we can loosen a bit.
  {
    ...
    const current    = config.screening.maxVolatility ?? 10.0; // no config key yet — default ceiling
  ...
  if (changes.minFeeActiveTvlRatio   != null) s.minFeeTvlRatio   = changes.minFeeActiveTvlRatio;
